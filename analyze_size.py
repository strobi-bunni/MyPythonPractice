...
def _create_report_row(
    ...
    if folder_first:
        ...
        if various_folder_num:
            ...
            yield SizeReportRow(
                fi.path / MISC_FOLDERS.format(various_folder_num),
                various_folder_size,
                various_folder_size / total_size,
                various_folder_size / current_level_size,
                "various_folders",
            )
        ...
        if various_file_num:
            ...
            yield SizeReportRow(
                fi.path / MISC_FILES.format(various_file_num),
                various_file_size,
                various_file_size / total_size,
                various_file_size / current_level_size,
                "various_files",
            )
    else:
    ...
    for row in rows:
        match row.row_type:
            case "file" | "various_files":
                path_repr = (
                    simply_path(row.path.relative_to(home_path))
                    if simply
                    else row.path.relative_to(home_path).as_posix()
                )
            case "folder" | "various_folders":
                path_repr = (
                    simply_path(row.path.relative_to(home_path), True)
                    if simply
                    else row.path.relative_to(home_path).as_posix() + "/"
                )
            case _:  # 'root'
