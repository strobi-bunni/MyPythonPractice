...
def format_differ_line(d: DifferLine, source_length: int, dest_length: int, colored=True, show_line_no=True):
    source_line_no_str = (
        int_right_align(d.source_line_no, int_str_length(source_length))
        if d.source_line_no
        else " " * int_str_length(source_length)
    )
    dest_line_no_str = (
        int_right_align(d.dest_line_no, int_str_length(dest_length))
        if d.dest_line_no
        else " " * int_str_length(dest_length)
    )
...
if __name__ == "__main__":
    ...
    for item in unified_diff(source_text, dest_text, n=args.n):
        if isinstance(item, DifferGroupHeader):
            print(
                format_differ_group_header(
                    item, lines_source, lines_dest, show_line_no=args.show_line_no, colored=args.colored
                )
            )
        else:
            print(
                format_differ_line(item, lines_source, lines_dest, show_line_no=args.show_line_no, colored=args.colored)
            )
