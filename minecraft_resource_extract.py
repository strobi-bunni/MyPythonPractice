...
if __name__ == '__main__':
    ...
    parser_extract = subparsers.add_parser('extract', help='파일을 추출한다',
                                           description='1개 이상의 대상 파일을 추출한다')
    parser_extract.add_argument('path', nargs='*', type=str, help='파일의 경로', default=['*'])
    parser_extract.add_argument('-mv', '--minecraft-version', type=str, help='마인크래프트 버전', metavar='X.XX',
                                default=get_latest_version())
    parser_extract.add_argument('-o', '--output', help='출력할 파일의 경로', required=True)

    parser_query = subparsers.add_parser('query', help='파일을 찾는다',
                                         description='대상 필터를 사용해서 파일을 찾는다')
    parser_query.add_argument('path', nargs='*', type=str, help='파일의 경로', default=['*'])
    parser_query.add_argument('-mv', '--minecraft-version', type=str, help='마인크래프트 버전', metavar='X.XX',
                              default=get_latest_version())
