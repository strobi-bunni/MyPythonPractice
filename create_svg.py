...
def add_rect(
    parent: Element, x: Real, y: Real, width: Real, height: Real, options: Optional[Dict[str, Any]] = None
) -> Element:
    if options is None:
        options = {}
    options = {k: str(v) for (k, v) in options.items()}
    return SubElement(
        parent, "rect", attrib={"x": str(x), "y": str(y), "width": str(width), "height": str(height), **options}
    )
...
def add_text(parent: Element, x: Real, y: Real, text: Any, options: Optional[Dict[str, Any]] = None) -> Element:
    if options is None:
        options = {}
    options = {k: str(v) for (k, v) in options.items()}
    elem = SubElement(parent, "text", attrib={"x": str(x), "y": str(y), **options})
    elem.text = str(text)
    return elem
...
if __name__ == "__main__":
    ...
    root = Element(
        "svg",
        attrib={
            "width": str(block_size * total_rows + 2 * margin_size),
