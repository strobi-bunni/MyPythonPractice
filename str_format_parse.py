...
TYPE_CODE = {'b': 'BINARY', 'c': 'CHAR', 'd': 'DECIMAL', 'e': 'EXPONENTIAL', 'E': 'EXPONENTIAL_UPPER',
             'f': 'FLOAT', 'F': 'FLOAT_UPPER', 'g': 'GENERAL', 'G': 'GENERAL_UPPER', 'n': 'NUMBER',
             'o': 'OCTAL', 's': 'STRING', 'x': 'HEXADECIMAL', 'X': 'HEXADECIMAL_UPPER', '%': 'PERCENT'}
...
def parse_format_spec(s: str) -> StrFormat:
    ...
    if matches := FMT_SPEC_REGEX.match(s):
        return StrFormat(
            ...
            TYPE_CODE[matches['type']] if matches['type'] else None, matches
        )
    else:
